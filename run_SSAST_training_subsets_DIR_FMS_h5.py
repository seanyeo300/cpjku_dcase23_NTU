...
def load_and_modify_checkpoint(pl_module,num_classes=10):
        ...
        return pl_module
model_path = r"D:\Sean\github\cpjku_dcase23_NTU\SSAST\SSAST-Base-Patch-400.pth"
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.model = ASTModel(label_dim=config.n_classes,
             fshape=16, tshape=16, fstride=10, tstride=10,
             input_fdim=128, input_tdim=config.tdim, model_size='base',
             pretrain_stage=False, load_pretrained_mdl_path=model_path)
    ...
    def mel_forward(self, x):
        ...
        x = x.reshape(-1, old_shape[2])
        x = self.mel(x)
        # print(f"shape after mel transformation: {x.shape}") #add this print statement
        x = x.reshape(old_shape[0], 1, x.shape[1], x.shape[2])
        # print(f"shape after mel reshape: {x.shape}") #Add this print statement
        return x
...
if __name__ == '__main__':
    ...
    # general
    parser.add_argument('--project_name', type=str, default="NTU24_SSAST")
    parser.add_argument('--experiment_name', type=str, default="NTU_SSAST_FTtau_441K_FMS_DIR_2e-5_avgcls_h5")
    ...
    parser.add_argument('--tdim', type=int, default=138)  # time dimension of input tensor (of spectrogram)
