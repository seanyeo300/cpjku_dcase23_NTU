...
class PLModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # loss
        samples_loss = loss_cls+R_attn + R_negtropy
        ...
        if self.calc_device_info:
            ...
            for i, d in enumerate(devices):
                if samples_loss.dim() > 0:
                    results["devloss." + d] = results["devloss." + d] + samples_loss[i].item()
                else:
                    results["devloss." + d] = results["devloss." + d] + samples_loss.item()
                results["devcnt." + d] = results["devcnt." + d] + 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="FocusNet_PaSST_training_sub5")
