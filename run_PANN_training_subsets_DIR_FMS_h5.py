...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.model = Wavegram_Logmel_Cnn14(sample_rate=config.resample_rate,window_size=config.window_size, hop_size=config.hop_size, mel_bins=config.n_mels, fmin=config.fmin, fmax=config.fmax, classes_num=config.ASclass)
    ...
    def forward(self, x, mixstyle_p=None, mixstyle_alpha=None):
        x = self.model(x,mixstyle_p,mixstyle_alpha)
        return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat, embed = self.forward(x, self.config.mixstyle_p, self.config.mixstyle_alpha)
        labels = labels.long()
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.config.lr, betas=(0.9,0.999),weight_decay=self.config.weight_decay)
...
def train(config):
    ...
    wandb_logger = WandbLogger(
        ...
        notes="PANN Training for DCASE2024, incl subset rules.",
        tags=["DCASE24"],
    ...
    if "model" in checkpoint:
        ...
        state_dict = checkpoint  # Use directly if no nesting

    # Initialize the PL model
...
if __name__ == '__main__':
    ...
    # general
    parser.add_argument('--project_name', type=str, default="NTU24_PANNs")
    parser.add_argument('--experiment_name', type=str, default="NTU_PANNs_FTtau_32K_FMS_DIR_1e-4_adamw_h5")
    ...
    parser.add_argument('--subset', type=str, default="10")
    # model
    # parser.add_argument('--arch', type=str, default='passt_s_swa_p16_128_ap476')  # pretrained passt model
    parser.add_argument('--n_classes', type=int, default=10)  # classification model with 'n_classes' output neurons
    parser.add_argument('--ASclass', type=int, default=527)  # classification model with 'n_classes' output neurons
