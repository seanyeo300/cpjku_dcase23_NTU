...
from helpers.util.pos_embed import get_2d_sincos_pos_embed, get_2d_sincos_pos_embed_flexible, get_1d_sincos_pos_embed_from_grid
from helpers.util.misc import concat_all_gather
from helpers.util.patch_embed import PatchEmbed_new, PatchEmbed_org
...
class MaskedAutoencoderViTForClassification(MaskedAutoencoderViT):
    ...
    def forward(self, imgs, mask_ratio=0.8):
        emb_enc, mask, ids_restore, _ = self.forward_encoder(imgs, mask_ratio, mask_2d=self.mask_2d)
        emb_enc = emb_enc[:, 0]  # Extract the [CLS] token
        logits = self.classifier(emb_enc)
        return logits
...
def mae_vit_base_patch16_dec512d8b_for_classification(**kwargs):
    model = MaskedAutoencoderViTForClassification(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
