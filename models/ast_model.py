...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # use a pretrained models for finetuning
            ...
            # print(f"p_f_dim: {p_f_dim}, p_t_dim: {p_t_dim}")  # Original dimensions
            # print(f"f_dim: {f_dim}, t_dim: {t_dim}")          # Fine-tuning dimensions
            # print(f"p_num_patches: {p_num_patches}")
            ...
            # print(f"new_pos_embed.shape: {new_pos_embed.shape}") # Shape of the new pos embed
            self.v.pos_embed = nn.Parameter(torch.cat([self.v.pos_embed[:, :self.cls_token_num, :].detach(), new_pos_embed], dim=1))
            # print(f"self.v.pos_embed.shape: {self.v.pos_embed.shape}") # Shape of the final pos embed
    ...
    def finetuningcls(self, x):
        ...
        if self.cls_token_num == 2:
            ...
            cls_tokens = self.v.cls_token.expand(B, -1, -1)
            x = torch.cat((cls_tokens, x), dim=1)
        # print(f"Shape of x before pos_embed: {x.shape}")
        # print(f"Shape of pos_embed: {self.v.pos_embed.shape}")
        x = x + self.v.pos_embed
        x = self.v.pos_drop(x)
