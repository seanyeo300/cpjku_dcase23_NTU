...
from datasets.dcase24_ntu_teacher_v4 import ntu_get_training_set_dir, ntu_get_test_set, ntu_get_eval_set, open_h5, close_h5, ntu_get_sub_training_set_dir, ntu_get_test_sub_set
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        #                   'street_pedestrian', 'street_traffic', 'tram'] #8A
        # self.label_ids = ['Bus', 'Park', 'Street','SubwayStation', 'Subway']
        # self.label_ids = ["airport", "public_square",  "street_pedestrian","metro", "metro_station"] #5A
        # self.label_ids =["airport", "bus", "public_square", "street_pedestrian", "shopping_mall", "metro", "metro_station", "tram"]
        # self.label_ids =["bus", "street_pedestrian","metro", "metro_station", "park"]
        # self.label_ids = ["airport", "public_square", "street_pedestrian","metro", "tram"] #TAU5D
        # self.label_ids = ["street_traffic", "park", "bus","shopping_mall", "metro_station"] #TAU5E
        self.label_ids = ["airport", "street_pedestrian", "public_square"] #TAU5_3
        # ['Bus', 'Cafe', 'Car', 'CrowdedIndoor', 'Elevator', 'Kitchen','Park', 'ResidentialArea', 'Restaurant', 'Restroom', 'Street','SubwayStation', 'Subway'] #CSfull
        # self.label_ids = ['bus', 'tram', 'metro', 'public_square', 'street_traffic'] #5B
        # self.label_ids = ['bus', 'tram', 'metro', 'public_square', 'street_traffic', 'airport', 'shopping_mall', 'street_pedestrian'] # 8B
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': head_params, 'lr': 1e-3},             # High learning rate for classifier heads
...
def train(config):
    ...
    # get_training set already has logic to handle dir_prob=0
    
    # train_dataset = ntu_get_training_set_dir(config.subset, config.dir_prob, hf_in, hmic_in)
    # print(f"Dataset length before DataLoader: {len(train_dataset)}")
    # assert len(train_dataset) > 0, "The dataset is empty!"
    train_dl = DataLoader(dataset=ntu_get_sub_training_set_dir(config.subset, config.dir_prob, hf_in, hmic_in),
    ...
    #                      pin_memory=True,
    #                      )
    test_dl = DataLoader(dataset=ntu_get_test_sub_set(hf_in),
                         worker_init_fn=worker_init_fn,
                         num_workers=config.num_workers,
                         batch_size=config.batch_size,
                         pin_memory=True,
...
def evaluate(config):
    ...
    # Open h5 file once
    hf_in = open_h5('h5py_audio_wav')
    # eval_hf = open_h5('h5py_cochl_test_mel_10s') # only when obtaining pre-computed train
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="CPJKU_passt_TAU3sub5_slow_fast_FGlr_6e-5_h5") # Changed helper file, subset, number of classes and label ids?
    ...
    parser.add_argument('--subset', type=str,default="5sub3") # cochlsub10, "cochlOLsub10", cochlNoOLsub10
    # model
    parser.add_argument('--arch', type=str, default='passt_s_swa_p16_128_ap476')  # pretrained passt model
    parser.add_argument('--n_classes', type=int, default=3)  # classification model with 'n_classes' output neurons
